...
#export CICD_TCI_TEMPLATE_DIR=`(cd "${DIRSCRIPT}/../../template/tci" ; pwd)
...
function render_engine_variables_request_body() {
  ...
  "${APPLICATION_TYPE_BUSINESSWORKS}")
    ...
    #request_body="$(render_tci_request_body "${DIRSCRIPT}/../../template/tci/businessworks-engine-variables.json")"
...
function deploy_app() {
  ...
  separatelog
  local temp_app_id
  temp_app_id="$(create_app_force_overwrite "${temp_app_name}" "${artifact}" "${manifest}")"
  echoerr Error code: $?
  
  # 2023/10/13: exit if deployment failed (for example 504)
  if [ "${temp_app_id}" = "" ] ; then
  ...
  separatelog
  #request_body="$(render_tci_request_body "${DIRSCRIPT}/../../template/tci/user-defined-variables.json")"
  ...
  separatelog
  #request_body="$(render_tci_request_body "${DIRSCRIPT}/../../template/tci/app-attributes.json")"
