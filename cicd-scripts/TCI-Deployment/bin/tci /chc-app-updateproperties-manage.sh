...
function usage() {
   ...
   echoerr "       ${SCRIPT} ${ACTION_Update} <app-name> <app-properties.json> <manifest.json>"
...
function render_engine_variables_request_body() {
  ...
  "${APPLICATION_TYPE_BUSINESSWORKS}")
    ...
    local Engine_properties="$engine_json"
...
function deploy_app() {
  ...
  bash ${DIRSCRIPT}/chc-updatecustomproperties.sh "${temp_app_name}" "${TCI_TOKEN}"
  separatelog
  export app_name
  local app_id
...
################################################################################
function update_app_properties() {

  local app_name="${1}"
  local app_type="${2}"
  local application_properties_json_path="${3}"
  local apps="${4}"
  ...
  if [[ -f "${application_properties_json_path}" ]]; then
    request_body="$(<"${application_properties_json_path}")"
    # update temp app variables declared in application-properties.json file
    update_app_variables "${app_id}" "${request_body}"
    separatelog
    wait_for_stopped_status "${app_id}" "${app_name}"
  else
    echoerr "File ${application_properties_json_path} not found - omitting update app variables ..."
  ...
  request_body="$(render_engine_variables_request_body "${app_type}")"
  separatelog
  update_engine_variables "${app_id}" "${request_body}"
  separatelog
  wait_for_stopped_status "${app_id}" "${app_name}"
  ...
  request_body="$(render_tci_request_body "${DIRSCRIPT}/../../template/user-defined-variables.json")"
  separatelog
  create_user_defined_variables "${app_id}" "${request_body}"
  separatelog
  wait_for_stopped_status "${app_id}" "${app_name}"
  ...
  if [ -n "${agent_key}" ] ; then
    separatelog
    update_hybrid_agent_access_key "${app_id}" "${agent_key}"
    separatelog
    wait_for_stopped_status "${app_id}" "${app_name}"
  fi  
  ...
  request_body="$(render_tci_request_body "${DIRSCRIPT}/../../template/app-attributes.json")"
  separatelog
  update_app_attributes "${app_id}" "${request_body}"
  separatelog
  wait_for_stopped_status "${app_id}" "${app_name}"
  ...
  #  get_tci_token
  #  if [ -z "${TCI_TOKEN}" ]; then
  #      echoerr "Failed to retrieve TCI token"
  #      exit 1
  #  fi
  #  separatelog

    # Debug output before calling the script
   # echoerr "Calling Update-engine-custom.sh with app_name=${app_name} and TCI_TOKEN=${TCI_TOKEN}"

    # Update the custom properties
   # bash "${DIRSCRIPT}/Update-engine-custom.sh" "${app_name}" "${TCI_TOKEN}"
   # if [ $? -ne 0 ]; then
   #     echoerr "Failed to update custom properties for app ${app_name}"
   #     exit 1
   # fi
   # separatelog

    #echoerr "Updated properties for app ${app_name}"
}
#########################################################################
...
function main() {
  ...
  "${ACTION_Update}")
    ...
    update_app_properties "${app_name}" "${app_type}" "${propfile}" "${apps}"
    ;;   
  "${ACTION_SHOW_PUBLIC_ENDPOINTS}")
