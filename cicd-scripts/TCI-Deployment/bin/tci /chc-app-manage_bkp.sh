...
# # Temporarily Modified the instance count to zero to do SIT deployment.
# export APP_CONF_INSTANCECOUNT=${APP_CONF_INSTANCECOUNT:0}
...
function stop_app() {
  ...
  if [[ "${app_id}" == "${NOT_FOUND}" ]]; then
    ...
    scale_app "${app_id}" "0"
    #invoke_tci_v1_rest_api "200" "POST" "apps/${app_id}/stop"
  fi
}
...
function main() {
  ...
  "${ACTION_SHOW_ENDPOINTS}")
    ...
    show_app_endpoints "${app_name}" "${apps}"
    ;;
  "${ACTION_SHOW_PUBLIC_ENDPOINTS}")
    ...
    show_app_public_endpoints "${app_name}" "${apps}"
    ;;
  "${ACTION_SHOW_PUBLIC_ENDPOINTS_PATH}")
    ...
    show_app_public_endpoints_path "${app_name}" "${apps}"
    ;;
  "stop")
    echoerr "app_name=${app_name}"
    echoerr "app_type=${app_type}"
