...
for ((index=0; index<$shared_modules_count; index++)); do
    ...
    cd ${WORKSPACE}/src/${INTERFACE_NAME}
    ...
    sharedmodule_path="${WORKSPACE}/src/${INTERFACE_NAME}/$repo_name/src"
    ...
    cp -r ${WORKSPACE}/src/${INTERFACE_NAME}/$repo_name/src/* ${WORKSPACE}/src/${INTERFACE_NAME}
    rm -rf ${repo_name}
    ls -l
done
